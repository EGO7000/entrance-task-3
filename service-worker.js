    ...
    const promise = deleteObsoleteCaches()
        .then(() => {
            ...
            /* Развёрнутый ответ (из документации):
                ...
                как на текущего клиента, так и на всех других активных клиентов.
                ---
...
self.addEventListener('fetch', event => {
    ...
    /* Развёрнутый ответ:
        ...
        2. Если у нас SPA-приложение, то url.pathname всегда будет содержать "/",
            ...
            Также вопрос к файлам, которые отдаются по динамической ссылке, как например,
            делает сервис: https://api.giphy.com/v1/gifs/search?limit=10&lang=ru&q=a&api_key=dc6zaTOxFJmzC".
        Замечание:
